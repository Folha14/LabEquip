...
namespace LabEquip.Models
{
    public class HelperUtilizador : HelperBase
    {
        ...
        public int getTotalUtilizadoresAtivos()
        {
            int totalAtivos = 0;
            SqlCommand comando = new SqlCommand();
            SqlConnection conexao = new SqlConnection(ConetorHerdado);
            ...
            try
            {
                comando.CommandType = CommandType.StoredProcedure;
                comando.Connection = conexao;
                comando.CommandText = "QUtilizador_GetTotalAtivos";

                conexao.Open();
                var resultado = comando.ExecuteScalar();
                if (resultado != null && resultado != DBNull.Value)
                {
                    totalAtivos = Convert.ToInt32(resultado);
                }
            }
            catch (Exception ex)
            {
                // Se der erro, conta manualmente
                totalAtivos = list().Count(u => u.Ativo);
            }
            finally
            {
        ...
        public string getUtilizadorMaisRecente()
        {
            string utilizadorRecente = "Nenhum";
            SqlCommand comando = new SqlCommand();
            SqlConnection conexao = new SqlConnection(ConetorHerdado);
            ...
            try
            {
                comando.CommandType = CommandType.StoredProcedure;
                comando.Connection = conexao;
                comando.CommandText = "QUtilizador_GetMaisRecente";
                ...
                {
                    DataRow linha = dt.Rows[0];
                    DateTime dataRegisto = Convert.ToDateTime(linha["dthrRegisto"]);
                    utilizadorRecente = linha["nome"].ToString() + " (" + dataRegisto.ToString("dd/MM/yyyy") + ")";
                }
            }
            catch (Exception ex)
            {
                // Se der erro, pega do método list()
                var usuarios = list().OrderByDescending(u => u.DthrRegisto).FirstOrDefault();
                if (usuarios != null)
                {
                    utilizadorRecente = usuarios.Nome + " (" + usuarios.DthrRegisto.ToString("dd/MM/yyyy") + ")";
                }
            }
            finally
            {
            ...
            return utilizadorRecente;
        }

        public string getContagemNiveis()
        {
            string contagem = "N/A";
            SqlCommand comando = new SqlCommand();
            SqlConnection conexao = new SqlConnection(ConetorHerdado);
            ...
            try
            {
                comando.CommandType = CommandType.StoredProcedure;
                comando.Connection = conexao;
                comando.CommandText = "QUtilizador_GetContagemNiveis";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    int utilizadores = Convert.ToInt32(linha["Utilizadores"]);
                    int administradores = Convert.ToInt32(linha["Administradores"]);

                    contagem = $"Admins: {administradores} | Users: {utilizadores} ";
                }
            }
            catch (Exception ex)
            {
                // Se der erro, conta manualmente
                var usuarios = list().Where(u => u.Ativo);
                int admins = usuarios.Count(u => u.NivelAcesso == 2);
                int users = usuarios.Count(u => u.NivelAcesso == 1);

                contagem = $"Admins: {admins} | Users: {users} ";
            }
            finally
            {
                conexao.Dispose();
